...
static char *make_choices_path(char *path, char *leaf, int create);
...
char *choices_find_path_load(char *leaf)
{
	...
	if (!program_name)
	{
		fprintf(stderr, "choices_find_path_load: not initialised\n");
		return NULL;
	}
	...
	if (!choices_path)
	{
		path = make_choices_path(default_user_choices(), leaf, 0);
		if (path && exists(path))
			return path;
	...
	while (choices_path)
	{
		path = make_choices_path(choices_path, leaf, 0);
		if (path && exists(path))
			return path;
...
char *choices_find_path_save(char *leaf)
{
	...
	if (choices_path)
		return make_choices_path(choices_path, leaf, 1);
	else
		return make_choices_path(default_user_choices(), leaf, 1);
}
...
static char *make_choices_path(char *path, char *leaf, int create)
{
	...
	strcpy(path_buffer + path_len + 1, program_name);
	if (create)
		mkdir(path_buffer, S_IRWXU | S_IRWXG | S_IRWXO);
